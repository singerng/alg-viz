...
class Turing {
  constructor(page, start, accept, transitions, initial) {
    ...
    this.steps = new _Var("Steps", 0);
    this.head = new _Var("Tape Head", 0);
    this.state = new _Var("State", start);
    this.accept = new _Var("Accept", accept.length > 0 ? accept.join(", ") : "none");
    this.status = new _Var("Status", "okay");
    ...
    for (var i = 0; i < initial.length; i++) {
      this.tape.set_object(new _Var("", initial[i], false, true, true), i);
    }
  ...
  *execute() {
    ...
    while (true) {
      ...
      if (lookup) {
        ...
        this.status.value = "found";

        yield;

        this.state.value = lookup.record.new;
        this.tape.set_object(new _Var("", lookup.record.newsym), this.head.value);
